...
from pzemdt import readAcPZEM         # power meter serial comm
from algdt import motorStatsAppend,motorStats,calAlgInit              # algorithms for alerts
import pubScribedt
...
def readPower() :
    ...
    for chan in range(0, len(chanNames)) :
        [voltage[chan], amperage[chan], power[chan], energy[chan], frequency[chan], powerFactor[chan], \
                alarmStatus[chan]] = readAcPZEM(chanPorts[chan], chanAddrs[chan])
        ...
        if (power[chan] > chanOnThresholds[chan]) :
            ...
            motorStatsAppend(chan, power[chan])
            ...
            hdr, returnStr, alertMsg = motorStats(chan, chanNames, rt, tInterval)
            ...
            if returnStr!="" :
                ...
                pubScribedt.pubRecord(pubScribedt.CSV_FILE, topic, returnStr, hdr)
...
def sendAlert(subj, alertMsg) :
    ...
    pubScribedt.pubRecord(pubScribedt.EMAIL_SMS, topic, alertMsg)
...
def sendStatus() :
    ...
    pubScribedt.pubRecord(pubScribedt.EMAIL_SMS, topic, statusMsg)
...
def energyLog():
    ...
    pubScribedt.pubRecord(pubScribedt.CSV_FILE, topic, s, hdr)
...
if __name__ == '__main__':
    ...
    if statusMsgEnabled :
        ...
        clearDown()
        topic = "energyMaster/Status"
        pubScribedt.pubRecord(pubScribedt.EMAIL_SMS, topic, "Program start")
    """
    ...
    pubScribedt.disconnectPubScribe()
